...
struct DecodeTAF: AsyncParsableCommand {
  ...
  @Option(
    name: [.customLong("taf-url"), .short],
    help: "The URL to load the TAF CSV from.",
  ...
  private func printTAF(_ taf: TAF) {
    ...
    if !taf.temperatures.isEmpty {
      let temps = taf.temperatures.map { TAF.Temperature.FormatStyle.temperature.format($0) }
        .joined(separator: ", ")
  ...
  private func printGroup(_ group: TAF.Group) {
    ...
    if !group.conditions.isEmpty {
      lprint(
        "  Conditions: \(group.conditions, format: .list(memberStyle: .condition, type: .and))"
      )
    }
    ...
    if !group.turbulence.isEmpty {
      let turbulence = group.turbulence.map { Turbulence.FormatStyle.turbulence.format($0) }.joined(
        separator: ", "
      )
    ...
    if let altimeter = group.altimeter {
      lprint(
        "  Altimeter: \(altimeter.measurement, format: .measurement(width: .abbreviated, usage: .asProvided))"
      )
    }
  }
...
extension Errors: LocalizedError {
  public var errorDescription: String? {
    switch self {
      ...
      case .badTAF(let raw): "Couldn’t parse TAF: \(raw)"
    }
  }
