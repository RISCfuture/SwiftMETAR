      ...
      switch value {
        ...
        case .variable(let speed, let headingRange):
          if let headingRange {
            ...
            return String(
              localized:
                "\(heading1, format: directionFormat)–\(heading2, format: directionFormat) at \(speed.measurement, format: speedFormat)",
              comment: "winds (heading–heading at speed)"
            )
          }
          return String(
        ...
        case .direction(let direction, let speed, let gust):
          ...
          if let gust {
            return String(
              localized:
                "\(directionM, format: directionFormat) at \(speed.measurement, format: speedFormat) gusting \(gust.measurement, format: speedFormat)",
              comment: "winds (heading at speed gusting)"
            )
          }
          return String(
            localized:
        ...
        case .directionRange(let direction, let headingRange, let speed, let gust):
          ...
          if let gust {
            return String(
              localized:
                "\(directionM, format: directionFormat) at \(speed.measurement, format: speedFormat) gusting \(gust.measurement, format: speedFormat) (\(heading1, format: directionFormat)–\(heading2, format: directionFormat))",
              comment: "winds (heading at speed gusting, variable heading–heading)"
            )
          }
          return String(
            localized:
              "\(directionM, format: directionFormat) at \(speed.measurement, format: speedFormat) (\(heading1, format: directionFormat)–\(heading2, format: directionFormat))",
            comment: "winds (heading at speed, variable heading–heading)"
          )
      }
    }
  }
...
extension FormatStyle where Self == Wind.FormatStyle {
  public static var wind: Self { .init() }

  public static func wind(
    directionFormat: Measurement<UnitAngle>.FormatStyle? = nil,
    speedFormat: Measurement<UnitSpeed>.FormatStyle? = nil
  ) -> Self {
    zipOptionals(directionFormat, speedFormat).map { .init(directionFormat: $0, speedFormat: $1) }
      ?? directionFormat.map { .init(directionFormat: $0) } ?? speedFormat.map {
        .init(speedFormat: $0)
      } ?? .init()
  }
}
