    ...
    describe("periodic precipitation amount") {
      it("parses a '60217' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.periodicPrecipitationAmount(period: 6, amount: 2.17))
        )
      }
      ...
      it("parses a '60000' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.periodicPrecipitationAmount(period: 6, amount: 0))
        )
      }
      ...
      it("parses a '6////' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.periodicPrecipitationAmount(period: 6, amount: nil))
        )
      }
    }
  }
