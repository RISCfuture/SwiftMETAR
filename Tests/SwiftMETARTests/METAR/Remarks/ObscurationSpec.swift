    ...
    override class func spec() {
        describe("obscuration") {
            it("parses a 'FG SCT000' remark") {
                ...
                expect(observation.remarks.map(\.remark)).to(contain(.obscuration(type: .fog, amount: .scattered, height: 0)))
            }
            ...
            it("parses a 'FU BKN020' remark") {
                ...
                expect(observation.remarks.map(\.remark)).to(contain(.obscuration(type: .smoke, amount: .broken, height: 2000)))
            }
        }
    }
}
