    ...
    describe("variable prevailing visibility") {
      it("parses a 'VIS 1/2V1 1/2' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.variablePrevailingVisibility(low: 1 / 2 as Ratio, high: 3 / 2 as Ratio))
        )
      }
      ...
      it("parses a 'VIS 1/2V5' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.variablePrevailingVisibility(low: 1 / 2 as Ratio, high: 5 as Ratio))
        )
      }
      ...
      it("parses a 'VIS 2V4' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.variablePrevailingVisibility(low: 2 as Ratio, high: 4 as Ratio))
        )
      }
    }
  }
