...
class TornadicActivitySpec: AsyncSpec {
    override class func spec() {
        describe("tornadic activity") {
            it("parses a 'TORNADO B13 6 NE MOV W' remark") {
                ...
                expect(observation.remarks.map(\.remark)).to(contain(.tornadicActivity(type: .tornado, begin: begin, end: nil, location: .init(direction: .northeast, distance: 6), movingDirection: .west)))
            }
            ...
            it("parses a 'WATERSPOUT E13 6 W' remark") {
                ...
                expect(observation.remarks.map(\.remark)).to(contain(.tornadicActivity(type: .waterspout, begin: nil, end: end, location: .init(direction: .west, distance: 6), movingDirection: nil)))
            }
            ...
            it("parses a 'FUNNEL CLOUD B1213 12 NW' remark") {
                ...
                expect(observation.remarks.map(\.remark)).to(contain(.tornadicActivity(type: .funnelCloud, begin: begin, end: nil, location: .init(direction: .northwest, distance: 12), movingDirection: nil)))
            }
        }
    }
}
