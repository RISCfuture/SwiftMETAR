    ...
    override class func spec() {
        describe("variable sky condition") {
            it("parses a 'BKN014 V OVC' remark") {
                ...
                expect(observation.remarks.map(\.remark)).to(contain(.variableSkyCondition(low: .broken, high: .overcast, height: 1400)))
            }
            ...
            it("parses a 'BKN V OVC' remark") {
                ...
                expect(observation.remarks.map(\.remark)).to(contain(.variableSkyCondition(low: .broken, high: .overcast, height: nil)))
            }
        }
    }
}
