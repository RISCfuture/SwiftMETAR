    ...
    describe("observed visibility") {
      it("parses a 'TWR VIS 1 1/2' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.observedVisibility(source: .tower, distance: 3 / 2 as Ratio))
        )
      }
      ...
      it("parses a 'SFC VIS 1/4' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.observedVisibility(source: .surface, distance: 1 / 4 as Ratio))
        )
      }
      ...
      it("parses a 'TWR VIS 2' remark") {
        ...
        expect(observation.remarks.map(\.remark)).to(
          contain(.observedVisibility(source: .tower, distance: 2 as Ratio))
        )
      }
    }
  }
