...
struct WindShiftTests {
  ...
  func parsesAWSHFT30Remark() async throws {
    ...
    #expect(
      observation.remarks.map(\.remark).contains(.windShift(time: date, frontalPassage: false))
    )
  }
  ...
  func parsesAWSHFT30FROPARemark() async throws {
    ...
    let observation = try await METAR.from(string: string)
    #expect(
      observation.remarks.map(\.remark).contains(.windShift(time: date, frontalPassage: true))
    )
  }
}
