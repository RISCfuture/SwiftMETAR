    ...
    override func spec() {
        describe("visibility") {
            it("parses fractional visibilities < 1 SM") {
                ...
                expect(visibility).to(equal(.equal(.statuteMiles(3, 4))))
            }
            ...
            it("parses fractional visibilities > 1 SM") {
                ...
                let visibility = try! METAR.from(string: string).visibility
                expect(visibility).to(equal(.equal(.statuteMiles(3, 2))))
            }
            ...
            it("parses whole visibilities > 1 SM") {
                ...
                let visibility = try! METAR.from(string: string).visibility
                expect(visibility).to(equal(.equal(.statuteMiles(3))))
            }
            ...
            it("parses visibilities < 1/4 SM") {
                ...
                let visibility = try! METAR.from(string: string).visibility
                expect(visibility).to(equal(.lessThan(.statuteMiles(1, 4))))
            }
            ...
            it("parses visibilities ≥ 10 SM") {
                ...
                let visibility = try! METAR.from(string: string).visibility
                expect(visibility).to(equal(.greaterThan(.statuteMiles(10))))
            }
