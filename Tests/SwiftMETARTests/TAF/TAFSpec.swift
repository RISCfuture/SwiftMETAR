  ...
  override class func spec() {
    ...
    describe("during") {
      it("returns the conditions active at a time") {
        ...
        expect(forecast.during(date)).to(
          equal(
            .init(
      ...
      it("parses a TAF without a forecast time") {
        ...
        expect(forecast.groups[0].period).to(
          equal(
            .range(
              .init(
        ...
        expect(forecast.groups[1].period).to(
          equal(
            .becoming(
              .init(
        ...
        expect(forecast.groups[2].period).to(
          equal(
            .temporary(
              .init(
        ...
        expect(forecast.groups[3].period).to(
          equal(.from(Date().this(day: 26, hour: 18, minute: 30)!))
        )
      }
      ...
      it("parses group comments") {
        ...
        expect(forecast.groups[1].remarks.map(\.remark)).to(
          equal([
        ...
        expect(forecast.groups[4].remarks.map(\.remark)).to(
          contain(.noAmendmentsAfter(Date().this(day: 26, hour: 5)!))
        )
        expect(forecast.groups[4].remarks.map(\.remark)).to(
          contain(.next(Date().this(day: 26, hour: 15)!))
        )
