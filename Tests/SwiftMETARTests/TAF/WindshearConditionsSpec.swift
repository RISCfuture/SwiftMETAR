...
struct WindshearConditionsTests {
  ...
  func `parses windshear conditions`() async throws {
    ...
    #expect(!forecast.groups[0].windshearConditions)
    #expect(!forecast.groups[1].windshearConditions)
    #expect(forecast.groups[2].windshearConditions)
    #expect(!forecast.groups[3].windshearConditions)
  }
}
